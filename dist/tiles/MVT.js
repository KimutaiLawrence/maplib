...
function decodeVarint(buffer, endian = 'little') {
    let result = 0;
    let shift = 0;
...
function decodePolygon(values, buffer, startByteIndex, tileExtent) {
    const result = [];
...
function decodeLayer(pbf) {
    const schema = {
        1: { type: 'string', name: 'name' },
        2: { type: 'int64', name: 'version' },
        3: { type: 'int32', name: 'extent' },
        4: { type: 'int32', name: 'key_value' },
        15: { type: 'message', name: 'features' }
    };
    const keyValues = [];
    const features = [];
    let name = '';
...
function parseKeyValue(buffer, result) {
    const pbf = new Pbf(buffer);
    while (!pbf.end()) {
        const tag = pbf.readVarint();
...
function decodeFeature(buffer, keyValuesBuffer, extent) {
    const pbf = new Pbf(buffer);
    const result = {
        id: 0,
        type: 'Polygon',
...
function decodeTags(buffer, keyValuesBuffer) {
    const result = {};
    if (!keyValuesBuffer) {
        return result;
    }
...
function decodeGeometry(buffer, extent) {
    const result = [];
    let x = 0;
    let y = 0;
    const points = [];
...
export function transformFeature(feature, tileX, tileY, zoom) {
    const transformed = {
        ...feature,
        coordinates: feature.coordinates.map(ring => ring.map(point => toLngLat(point, zoom, tileX, tileY)))
    };
    return transformed;
}
